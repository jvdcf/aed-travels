...
float Airport::distanceTo(float pLat, float pLon) const {
    ...
    float haversine_step1 = sinf(lat / 2) * sinf(lat / 2) +
                            cosf(this->getLatitude() * ((float) M_PI / 180.0)) * cosf(pLat * ((float) M_PI / 180.0)) *
                            sinf(lon / 2) * sinf(lon / 2);
...
uint16_t Airport::codeToHash(const std::string &code) {
    ...
    for (int i = 2; i >= 0; --i) {
        res <<= 5;
        res += (code[i] - 'A') & (32 - 1);
        // "& (32-1)": obtain the 5 Least Significant Bits. (32-1) = 31 = 0b11111
    }
    return res;
}
